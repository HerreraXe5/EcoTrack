...
export default function DashboardScreen() {
  ...
  const cargarDatos = async () => {
    try {
      ...
      if (resRegistros.ok) {
        ...
        console.log('✅ Registros:', dataRegistros.length);
      }
  ...
  const getNivel = (kg: number) => {
    if (kg < 5) return '🌱 Semilla';
    if (kg < 20) return '🌳 Árbol';
    if (kg < 50) return '🌲 Bosque';
    return '🌍 Guardián Planetario';
  };
      ...
      <ScrollView contentContainerStyle={styles.scrollContainer} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.header}>
          ...
          <View style={styles.nivelBadge}>
            <Text style={styles.nivelTexto}>{getNivel(pesoTotal)}</Text>
          </View>
        </View>
        ...
        <TouchableOpacity style={styles.botonPrincipal} onPress={() => router.push('/(tabs)/nuevo-pesaje')}>
          <Ionicons name="add-circle-outline" size={24} color="#FFFFFF" />
          <Text style={styles.botonPrincipalTexto}>Registrar Nuevo Pesaje</Text>
        </TouchableOpacity>

        <View style={styles.botonesSecundarios}>
          <TouchableOpacity style={styles.botonSecundario} onPress={() => router.push('/(tabs)/historial')}>
            <Ionicons name="list-outline" size={24} color="#115E3E" />
            <Text style={styles.botonSecundarioTexto}>Historial</Text>
          </TouchableOpacity>
          <TouchableOpacity style={styles.botonSecundario} onPress={() => router.push('/(tabs)/logros')}>
        ...
        {usuario?.rol === 'admin' && (
          <TouchableOpacity style={styles.botonAdmin} onPress={() => router.push('/(tabs)/admin-categorias')}>
...
const styles = StyleSheet.create({
  ...
  nivelBadge: { backgroundColor: '#D1F2E0', paddingHorizontal: 12, paddingVertical: 6, borderRadius: 20 },
  ...
  botonSecundario: {
    flex: 1, backgroundColor: '#FFFFFF', borderRadius: 12, height: 56,
    flexDirection: 'row', justifyContent: 'center', alignItems: 'center',
    borderWidth: 1, borderColor: '#BFE9D4', gap: 8,
  },
  botonSecundarioTexto: { color: '#115E3E', fontSize: 14, fontWeight: '600' },
  botonAdmin: {
    backgroundColor: '#1E40AF', borderRadius: 12, height: 48,
